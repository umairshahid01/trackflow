...
def inject_branding_css():
    ...
    # Subtle animated gradient + logo zoom/dock + slabs styling + admin button
    st.markdown(
        f"""
        <style>
        :root {{
            ...
            --card:#0b1220cc; /* translucent dark */
        }}
...
def show_logo():
    logo_b64 = read_image_as_base64(Path("assets/logo_placeholder.png"))
    if logo_b64:
        st.markdown(
            f"""
...
def view_home():
    ...
    # Floating visual Admin button (for looks)
    st.markdown(
        """
